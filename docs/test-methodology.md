...
### 23. 概率性断言 = 测试设计缺陷（2026-09-12 test_n_agents 随机红）

**背景**：`test_four_agent_cascade` 断言"N=4 应有 message 发言"。FakeAgent 在
meeting 阶段有 35% 概率写 freezing，而"全员首轮 freezing"概率 = 0.35⁴ =
**1.50%**——此时测试必失败（N=5 时 0.53%）。实测约每 10 次全量跑红一次。

**定性**：**测试装置问题，不是产品问题**——全员首轮 freezing 是合法协议路径
（引擎照常走完 af → RR → pass → concluded）。但断言把"典型路径"当成不变式了。

**方法**：
1. **测试可以断言不变式，不可以断言分布尾部**：写断言前先问"这个结论在
   装置的随机性下是否恒成立"。不成立 → 断言错，或装置要改。
2. **修装置优于挑种子**：本次修法是让 FakeAgent **首轮必写 message**——
   理由是**装置对齐生产**（真实 wake prompt 对首位发言者的指令就是"直接产出
   你的第一条视角分析"），而不是"把概率调到能过"。挑 seed 只治一次，改对齐
   治一类。
