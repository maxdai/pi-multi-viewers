...
| 模式 | 做法 | 产物规模（本仓库主 session 实测） | 适用 |
|---|---|---|---|
| `budget`（默认） | 从 compaction 边界起 + 折叠（丢 thinking、旧工具输出换省略标记、长参数截断）+ 按预算从尾部保留，更早的丢弃并写「上下文说明」preface | 806 条 / 0.89 MB / est ≈ 80k（真实 ≈132k） | 长会话**唯一可行** |
| `compaction` | 从最后 compaction 的 `firstKeptEntryId` 起，内容原样（不折叠） | 2894 条 / 5.66 MB（≈930k tokens） | 中小会话（零信息损失） |
| `full` | 全部条目 | 6741 条 / 15.07 MB | 小会话 / 验证 |
...
- fork 携带的是 session **原始条目**；主 pi 实际发送的上下文由压缩层在
  **渲染时**生成（不在条目里）——主 pi 实测每次请求 ≈185k tokens，而同一
  session 的原始条目文本 ≈930k tokens。
- 模型窗口 1M，每次请求含 384k completion 预留 → 消息预算 ≈664k tokens。
- 实测：`full` 首请求 1,303,280 tokens、`compaction` 934,630 tokens → 均被
  provider 400 拒绝；`pi --fork` 原生命令同样超窗（731,331 tokens）。
- `budget` 首唤实测 ≈132k（唤醒 1）→ 讨论完整收敛（3 视角 / 19 分钟）。

---

## 二、规模口径（单一事实源；三处互引）

1. **产物侧指纹 `forkSourceTokensEst`**：同一基准构建的确定性结果
   （三 agent 恒同值）——用于校验"产物是否同源 / 裁剪是否符合预期"，
   **不预测请求规模**；字符数 / 3 估算，字段名带 `Est` 即为提醒。
2. **消费侧规模（验收/成本基准）**：**唤醒 1 的第一次请求** `input +
   cacheRead`（含系统提示与工具定义）。引用必须带唤醒序号，否则数字不可比。
3. **校准比（est → 真实）**：唤醒 1 ≈ **1.66×**；唤醒中后期至 ~2.7×
   ——**不得固化为 2.0×**（不同测点值不同）。
...
| 方案 | 否因 | 重估触发 | 所在位置 |
|---|---|---|---|
| **F1** 首唤时把 tail id 从 `build_fork_source` 传给 `append_handoff_turns`（省一次重读） | ①收益仅 0.011s；②方案自败（保留重读兜底则被指瑕疵的推导代码一行未减）；③新增静默失败面（tail id 可能过期 → 接错节点）；④把 session 格式知识泄漏到 loop 层 | 源 ≥100MB 或 N≫3（内存 ≈2.5×文件大小×N） | `meeting_fs.append_handoff_turns` / `meeting_loop` 首唤路径 |
...
`meeting_fs` 的 fork 源区块（`read_fork_stats` / `_est_tokens` /
`_entry_text` / `_shrink_value` / `_fold_entry` / `_budget_entries` /
`build_fork_source` / `append_handoff_turns`）现为一个章节。满足任一条件时
再拆为独立模块（或 `meeting_fork.py`）：
...
- MC 增强路线未实现：现行 `budget` 刻意**不依赖任何扩展**（纯 pi 语义 +
  预算 + 折叠）。
