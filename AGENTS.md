...
**总原则（错误→制度化，用户 2026-09-09 定）**：出现错误后，AGENTS.md 要记的
不是"这个 bug 怎么修/这个具体方法是什么"，而是**什么制度缺失让它漏过、让
定位变慢**——补制度，保证下次同类错误在结构上不可能再犯。具体方法只作为
制度下的实例存在（挂靠不单列）。复盘必问四句：
1. **哪个特性/改动引入了这个错误面？它的隐含假设是什么**（如"agent 名是
   ASCII"）——引入时有没有补这个假设的边界测试？→ 制度：**特性落地必须
   显式盘点它打破的既有假设，并补对应边界测试**（不是"功能测过就行"）。
2. **定位用了正确的方法吗**？纯机制层 bug（git/文件/编码/状态机）必须用
   确定性复现（单测/小实验/装置），**禁止用真实 LLM e2e 碰运气**（不可控、
   慢、贵，只做最终确认）。→ 制度：bug 先按"机制层 vs LLM 行为层"分类选
   复现手段；机制层 3 分钟内无复现即写最小实验，不靠长跑试错。
3. **失败现场保留了吗**？测试失败删 log = 测试白跑，只剩断言消息无法
   定位。→ 制度：测试装置失败必须保留现场（目录+日志），成功才清理。
4. **改装置/用新 API 先验证了吗**？先查文档或写秒级最小实验确认 API 语义，
   不写完整代码跑长测试试错（_outcome 时序两次实测失败后改用
   sys.exc_info——先做 0.1s 实验即可知）。

（"反复遇到的问题必须总结出程序化应对并固化（测试装置/helper/文档），
不能总是再试一次碰运气"——方法条目如下，均挂在上述制度下）
...
5. **测试运行/观察分离（tests/run_tests.sh）**：层 1（默认）每次真跑 +
   ...
   `--force` 真跑。
6. **参数形态矩阵覆盖（二维）**：路径/名字类参数测试覆盖全部调用形态——
   维度 1 写法（绝对/./相对/裸名）；维度 2 **字符集（ASCII/中文）**——
   git quotepath 只转义非 ASCII（中文名拿到带引号路径 → list_my_messages
   恒空 → is_first 恒真 → freezing 卡死，2026-09-09 e2e 实测；纯机制
   层 bug 本可用单测抓住，viewers 中文名是产品核心，不是 exotic case）。
